...
# Globális változók az utolsó SAMPLE értékekhez
last_smp_T = None
last_smp_RH = None
last_smp_P = None
            ...
            try:
                ...
                while True:
                    ...
                    if sync == 0xA6:
                        if len(data) >= 14:
                            MET = data[0] | (data[1] << 8)
                            T, RH, P = struct.unpack('<fff', data[2:14])
                            global last_smp_T, last_smp_RH, last_smp_P
                            last_smp_T = T
                            last_smp_RH = RH
                            last_smp_P = P
                            print(f"{datetime.datetime.now().strftime('[%H:%M:%S]')} [SMP] MET={MET}  T={T:.2f}C  RH={RH:.1f}%  P={P:.1f}hPa")
                        else:
                            print(f"[SMP] RAW {data.hex().upper()}")
