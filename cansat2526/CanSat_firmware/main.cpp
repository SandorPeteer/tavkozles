...
static uint8_t g_prev_pack_global[5] = {0};
...
static uint8_t encode_dynamic_pack(const TeleSample &s, uint8_t *out)
{
  ...
  float P_round = floorf(P * 10.0f + 0.0001f) / 10.0f;

  // integer part
  int32_t P_int = (int32_t)P_round;
  uint32_t P_int_code = (uint32_t)(P_int - 822);
  if (P_int_code > 255)
    P_int_code = 255;

  // fractional part (0..9)
  uint32_t P_frac_code = (uint32_t)((P_round - (float)P_int) * 10.0f);
  if (P_frac_code > 9)
    P_frac_code = 9; // safety only
...
void loop()
{
  ...
  if (now - g_last_tele_ms >= 500)
  {
    ...
    // 2 + 1 + 1 + N*5 <= 48 → N max. 8, de ha elérjük a 8-at, flush
    if (g_tele_count >= MAX_SAMPLES_PER_PACKET)
    {
      telemetry_flush_packet();
    }
  ...
  // 4) Időnként (pl. 5 s-onként) kényszerített flush, hogy ne maradjanak bent minták
  static uint32_t last_flush_ms = 0;
  if (now - last_flush_ms >= 5000)
  {
    last_flush_ms = now;
    telemetry_flush_packet();
  }
